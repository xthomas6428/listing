...
if ( !class_exists("Listdo_Mobile_Menu") ) {
    class Listdo_Mobile_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'listdo');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'listdo');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'listdo');
                        break;
            ...
            $item_output .= '</a>';
            $item_output .= $args->has_children || $apus_mega_profile ? ' <span class="icon-toggle"><i class="ti-angle-down"></i></span>' : '';
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            ...
            if ( $apus_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    $post = $posts[0];
                    $content = apply_filters( 'listdo_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
                    $width  = $item->apus_width ? 'style="width:'.$item->apus_width.'px"':"";
                    return '<div class="sub-menu '.esc_attr($post->post_name).'" '.$width.'><div class="dropdown-menu-inner">'.$content.'</div></div>';
                }
            }
            return '';
        }
