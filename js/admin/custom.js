(function () {
	...
	jQuery(document).ready(function($) {
		...
		$('body').on('click', '.add-new-menu-price', function(e){
			e.preventDefault();
            var parent = $(this).closest('.group-field-item-content');
        ...
        // hours operation admin
        $('.is-admin .hours-field-wrapper .nav-tabs a').on('click', function(e){
        	e.preventDefault();
        	$('.is-admin .hours-field-wrapper .nav-tabs li').removeClass('active');
        	$(this).parent().addClass('active');
        	var e_id = $(this).attr('href');
        	$('.is-admin .hours-field-wrapper .tab-content .tab-pane').removeClass('active');
        	$(e_id).addClass('active');
        });

        // datetime
        if ( $( 'input.field-datetimepicker' ).length > 0 ) {
            ...
            if ( $(main_select).length > 0 ) {
                ...
                if ( val ) {
                    $.ajax({
                        ...
                        type:'POST',
                        dataType: 'html',
                        data:  'action=listdo_process_change_region&region_parent='+val+'&next='+next+'&name='+ name + '&security=' + listdo_opts.ajax_nonce
                    }).done(function(data) {
                    ...
                    if ( name == '_job_regions[]' || name == 'job_regions[]' ) {
                        html += '<label>' + listdo_opts.region_labels[next] +'</label>';
                    }
        ...
        // Category
        $('body').on('change', 'select#_job_c_category', function(){
            if ( $('#_job_c_type').length > 0 ) {
                var main_select = $('#_job_c_type');
                var val = $(this).val();
                ...
                $.ajax({
                    ...
                    data:{
                        ...
                        'category_parent': val,
                        'security': listdo_opts.ajax_nonce,
                    ...
                    main_con.find('#_job_c_type').remove();
                    main_con.append(data);
                });
            }
        });

        // Category change amenity
        $('body').on('change', 'select#_job_category', function(){
            if ( $('.fieldset-_job_amenities').length > 0 ) {
                var val = $(this).val();
                change_category_amenities(val);
            }
        });
        $('body').on('change', '.job-manager-term-checklist-_job_category input[name^=_job_category]', function(){
            if ( $('.fieldset-_job_amenities').length > 0 ) {
                var val = [];
                $.each($(".job-manager-term-checklist-_job_category input[name^=_job_category]:checked"), function(){            
                    val.push($(this).val());
                });
                change_category_amenities(val);
            }
        ...
        var change_category_amenities = function(val) {
            var main_con = $('.fieldset-_job_amenities');
            main_con.find('.field').addClass('loading');
        ...
        function listdo_get_listdo_flaticon_icons() {
            ...
            for( var i=0; i < files.length; i++ ){
                css = Array.prototype.map.call( files[i].sheet.cssRules, listdo_css_text ).join('\n');
                ...
                css.forEach(function( i ){
                    ...
                    if( i !== undefined && i.indexOf('/') == -1 ) {
                        ...
                        if ( class_name !== '' ) {
                            icons_load_call.push(class_name);
                        }
                    }
                });
            }
            
            all_loaded_icons.listdo = icons_load_call;
        }
        function listdo_css_text(x) {
            return x.cssText;
        }
        ...
        $( ".repeate_field_add_row" ).on('click', function(){
            var html = $($(this).data('row'));
            html.find( 'input.repeat-datepicker' ).each(function(){
                var input_id = $(this).attr('id');
                $(this).attr( 'id', input_id + number_increment );
            });

            $(this).closest('.wc-job-manager-repeater-rows').find('.repeater-fields-rows').append( html );

            $(this).closest('.wc-job-manager-repeater-rows').find( 'input.repeat-datepicker' ).each(function(){
                var input_id = $(this).attr('id');
                $('#'+input_id).datepicker({
                    minDate    : 0
                });
        ...
        function show_hide_listing_setting(val) {
            if ( val == 'page-listing.php' ) {
                $('#apus_page_listing_setting').show();
            } else {
                $('#apus_page_listing_setting').hide();
            }
        }
	});
