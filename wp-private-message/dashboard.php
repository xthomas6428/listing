...
<div class="box-list">
<h3 class="title"><i class="flaticon-chat-comment-oval-speech-bubble-with-text-lines"></i><?php echo esc_html__('Messages','listdo') ?></h3>
<div class="inner no-padding">
<?php if ( $loop->have_posts() ) { ?>
	<a href="javascript:void(0);" class="btn toggle-message-btn">
		<?php esc_html_e('Show messages box', 'listdo'); ?> <i class="fa fa-angle-down" aria-hidden="true"></i>
	</a>
				...
				<form id="search-message-form" class="search-message-form" action="" method="post">
					<div class="search-wrapper-message">
						<div class="form-group">
				            <input type="text" class="form-control" name="search" placeholder="<?php esc_attr_e( 'Search', 'listdo' ); ?>">
				        </div>
			        ...
			        <div class="filter-options">
			        	<ul class="list-options-action">
			        		<li><input id="search_read_all" type="radio" name="search_read" value="all" <?php checked($search_read, 'all'); ?>><label for="search_read_all"><?php esc_html_e('All', 'listdo'); ?></label></li>
			        		<li><input id="search_read_read" type="radio" name="search_read" value="read" <?php checked($search_read, 'read'); ?>><label for="search_read_read"><?php esc_html_e('Read', 'listdo'); ?></label></li>
			        		<li><input id="search_read_unread" type="radio" name="search_read" value="unread" <?php checked($search_read, 'unread'); ?>><label for="search_read_unread"><?php esc_html_e('Unread', 'listdo'); ?></label></li>
			        	</ul>
			        </div>
				</form>
				<div class="list-message-inner">
					...
					if ( $next_page <= $loop->max_num_pages ) { ?>
						<div class="loadmore-action">
							<a href="javascript:void(0);" class="loadmore-message-btn" data-paged="<?php echo esc_attr($next_page); ?>"><?php esc_html_e( 'Load more', 'listdo' ); ?></a>
						</div>
					<?php } ?>
				</div>
			</div>
...
<?php } else { ?>
	<div class="not-found"><?php esc_html_e('No message found', 'listdo'); ?></div>
<?php } ?>
</div>
</div>
